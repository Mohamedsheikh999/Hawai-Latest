...
const SignUpForm = ({ theme, onToggleForm }) => {
  ...
  return (
    <div className={`w-full max-w-md p-8 rounded-xl shadow-lg ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
      <h2 className={`text-2xl font-bold mb-6 ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>Create Account</h2>
      
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="name" className={`block text-sm font-medium mb-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
            Full Name
          </label>
        ...
        <div className="flex items-center">
          <input
            ...
            required
          />
          <label htmlFor="terms" className={`ml-2 text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
            I agree to the <a href="#" className={`underline ${theme === 'dark' ? 'text-emerald-400' : 'text-green-600'}`}>Terms</a> and <a href="#" className={`underline ${theme === 'dark' ? 'text-emerald-400' : 'text-green-600'}`}>Privacy Policy</a>
          </label>
        </div>
        ...
        >
          <span className="flex items-center justify-center">
            Create Account <FiArrowRight className="ml-2 transition-transform group-hover:translate-x-1" />
          </span>
        </Button>
      </form>
